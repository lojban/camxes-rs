...
use crate::peg::parsing::{Span, ParseError, ErrorKind, ParseResult, ParseNode};
        ...
        match self {
            ...
            Rule::Any => {
                if position < input.len() {
                    ParseResult(1, position + 1, Ok(vec![ParseNode::Terminal(Span(position, position + 1))]))
                } else {
                    ParseResult(1, position, Err(ParseError {
                        position,
            ...
            Rule::Literal(pattern) => {
                if input[position..].starts_with(pattern) {
                    ParseResult(1, position + pattern.len(), 
                        Ok(vec![ParseNode::Terminal(Span(position, position + pattern.len()))]))
                } else {
                    ParseResult(1, position, Err(ParseError {
                        position,
                        expression: self.clone(),
            ...
            Rule::NonTerminal(name) => {
                ...
                let output = match peg.rules.get(name).unwrap().parse(peg, input, position, depth + 1) {
                    ParseResult(cost, new_pos, Ok(matches)) => 
                        ParseResult(cost, new_pos, Ok(vec![ParseNode::NonTerminal(
                            name.clone(), Span(position, new_pos), matches)])),
                    ParseResult(cost, _, Err(inner)) => 
                        ParseResult(cost, position, Err(ParseError {
                            position,
                            expression: self.clone(),
                ...
                debug!("{}└{} {} @ {} -> {}", "│".repeat(depth),
                    if output.2.is_ok() { "ok" } else { "err" }, name, position,
                    output.1);
                output
            },
            ...
            Rule::Choice(choices) => {
                for choice in choices {
                    if let ParseResult(cost, new_pos, Ok(matches)) = choice.parse(peg, input, position, depth) {
            ...
            Rule::Not(expr) => match expr.parse(peg, input, position, depth) {
                ParseResult(_, _, Ok(m)) => ParseResult(1, position, Err(ParseError {
                    position,
                    expression: self.clone(),
            ...
            Rule::Class(symbols) => {
                if let Some(symbol) = symbols.iter().find(|s| input[position..].starts_with(s.as_str())) {
                    ParseResult(1, position + symbol.len(),
                        Ok(vec![ParseNode::Terminal(Span(position, position + symbol.len()))]))
                } else {
                    ParseResult(1, position, Err(ParseError {
                        position,
                        expression: self.clone(),

...
impl PEG {
    pub fn new(start: &str, grammar: &str) -> GrammarResult<Self> {
        ...
        match parser.parse(grammar) {
            ParseResult(_, _, Ok(tokens)) => {
                Transformer { source: grammar.trim() }
                    .build(start, tokens)
    ...
    pub fn parse(&self, input: &str) -> ParseResult {
        Rule::NonTerminal(self.start.clone())
            .parse(self, input, 0, 0)
    }
...
fn define_operators(gb: &mut RuleBuilder) {
    let spacing = gb.add_rule(SPACING, gb.zero_or_more(gb.choice(vec![
    ...
    for (name, symbol) in operators {
        gb.add_rule(name, gb.seq(vec![
            Rule::Literal(symbol.to_string()),
            spacing.clone(),
        ]));
    }
...
fn define_character_rules(gb: &mut RuleBuilder) {
    ...
    let range = gb.add_rule(RANGE, gb.seq(vec![
        Rule::NonTerminal(CHAR.to_string()),
        Rule::Literal("-".to_string()),
        Rule::NonTerminal(CHAR.to_string()),
    ]));

    gb.add_rule(CLASS_MEMBER, gb.choice(vec![
        range.clone(),
        Rule::NonTerminal(CHAR.to_string()),
    ]));
}
...
fn define_nonterminal_rules(gb: &mut RuleBuilder) {
    ...
    let primary = gb.add_rule(PRIMARY, gb.choice(vec![
        gb.seq(vec![
            identifier.clone(),
    ...
    let suffix = gb.add_rule(SUFFIX, gb.optional(gb.choice(vec![
        Rule::NonTerminal(QUESTION.to_string()),
        Rule::NonTerminal(STAR.to_string()),
        Rule::NonTerminal(PLUS.to_string()),
    ])));

    let prefix = gb.add_rule(PREFIX, gb.optional(gb.choice(vec![
        Rule::NonTerminal(AND.to_string()),
        Rule::NonTerminal(NOT.to_string()),
    ])));

    let sequence = gb.add_rule(SEQUENCE, gb.zero_or_more(gb.seq(vec![
        prefix.clone(),
        primary.clone(),
        suffix.clone(),
    ])));

    gb.add_rule(EXPR, gb.seq(vec![
        sequence.clone(),
        gb.zero_or_more(gb.seq(vec![
            Rule::NonTerminal(SLASH.to_string()),
            sequence.clone(),
        ])),
    ]));
}
...
fn define_grammar_rules(gb: &mut RuleBuilder) {
    let spacing = Rule::NonTerminal(SPACING.to_string());
    let definition = gb.add_rule(DEF, gb.seq(vec![
        Rule::NonTerminal(IDENT.to_string()),
        Rule::NonTerminal(ARROW.to_string()),
        Rule::NonTerminal(EXPR.to_string()),
    ]));

    gb.add_rule(TEXT, gb.seq(vec![
        spacing.clone(),
...
fn define_literal_rule(gb: &mut RuleBuilder, spacing: &Rule) -> Rule {
    ...
    gb.add_rule(LITERAL, gb.choice(vec![
        single_quoted,
        double_quoted,
    ]))
}

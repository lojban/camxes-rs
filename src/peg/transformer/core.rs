...
use crate::peg::grammar::{AND, ARROW, CHAR, CLASS, CLASS_MEMBER, DEF, DOT, EOF, EXPR, IDENT, LITERAL, LPAR, NOT, PLUS, PREFIX, PRIMARY, QUESTION, RANGE, RPAR, SEQUENCE, SLASH, SPACING, STAR, SUFFIX, TEXT};
use crate::peg::parsing::{ParseNode, Span};
use super::errors::TransformError;

type Result<T> = std::result::Result<T, TransformError>;

pub struct Transformer<'a> { pub source: &'a str }
...
impl Transformer<'_> {
    pub fn build(&self, start_rule: &str, cst: Vec<ParseNode>) -> Result<PEG> {
        match &cst[..] {
            ...
            [ParseNode::NonTerminal(n, _, _)] => Err(TransformError::CstShouldStartWithGrammar(format!("Found '{n}' instead!"))),
            _ => Err(TransformError::CstShouldOnlyHaveOneRoot("Invalid root structure".into())),
        }
    }

    fn build_grammar_rules(&self, tokens: &[ParseNode]) -> Result<Arc<HashMap<String, Rule>>> {
        let (rules, refs) = tokens.iter()
            .skip(1)
            .take_while(|t| !matches!(t, ParseNode::NonTerminal(n, _, _) if n == EOF))
            .try_fold((HashMap::new(), HashSet::new()), |(mut rules, mut refs), parse_node| {
        ...
        if !undefined.is_empty() {
            return Err(TransformError::AmbiguousNonTerminal(format!("Missing rules: [{}]!", undefined.join(", "))));
        }
        Ok(Arc::new(rules))
    }
    ...
    fn process_rule(&self, parse_node: &ParseNode) -> Result<(String, Rule, HashSet<String>)> {
        let [id, arrow, expr] = Self::get_tokens(DEF, parse_node)?.as_slice() else {
            return Err(TransformError::WrongNumberOfTokens("Definition needs 3 tokens".into()));
        };
    ...
    fn extract_identifier(&self, parse_node: &ParseNode) -> Result<String> {
        let result = Self::get_tokens(IDENT, parse_node)?
            ...
            .collect::<String>();
        if result.is_empty() { Err(TransformError::EmptyIdentifier) } else { Ok(result) }
    }
    ...
    fn convert_rule(&self, parse_node: &ParseNode) -> Result<(Rule, HashSet<String>)> {
        ...
        let (exprs, refs): (Vec<_>, HashSet<_>) = chunks.iter().try_fold((vec![], HashSet::new()), |(mut exprs, mut refs), chunk| {
    ...
    fn apply_prefix(&self, parse_node: &ParseNode, expr: Rule) -> Result<Rule> {
        Ok(match Self::get_tokens(PREFIX, parse_node)?.first() {
            ...
            Some(_) => return Err(TransformError::UnExpectedToken(format!("Expected {AND}/{NOT}"))),
            None => expr,
        })
    }
    ...
    fn convert_primary(&self, parse_node: &ParseNode) -> Result<(Rule, HashSet<String>)> {
        ...
        match tokens.as_slice() {
            ...
            _ => Err(TransformError::WrongNumberOfTokens("Primary needs 1 or 3 tokens".into())),
        }
    }

    fn unescape_char(&self, parse_node: &ParseNode) -> Result<String> {
        match parse_node {
            ParseNode::NonTerminal(name, Span(s, e), _) if name == CHAR =>
                Ok(self.source[*s..*e]
                    .replace("\\'", "'")
                    .replace("\\\"", "\"")
                    .replace("\\[", "[")
    ...
    fn convert_class(&self, parse_node: &ParseNode) -> Result<Rule> {
        ...
        let (parts, symbols) = tokens[1..tokens.len() - 2].iter()
            .try_fold((vec![], HashSet::new()), |(mut parts, mut symbols), member| {
    ...
    fn convert_range(&self, parse_node: &ParseNode) -> Result<Rule> {
        ...
        if tokens.len() != 3 {
            return Err(TransformError::WrongNumberOfTokens("Range needs 3 tokens".into()));
        }
        Ok(Rule::Range(self.unescape_char(&tokens[0])?, self.unescape_char(&tokens[2])?))
    }
    ...
    fn get_tokens<'a>(name: &str, parse_node: &'a ParseNode) -> Result<&'a Vec<ParseNode>> {
        match parse_node {
            ...
            ParseNode::NonTerminal(n, _, _) => Err(TransformError::UnExpectedToken(format!("Expected {name}, got {n}"))),
    ...
    fn get_name(parse_node: &ParseNode) -> Result<&str> {
        match parse_node {
            ...
            _ => Err(TransformError::UnExpectedToken("Expected non-terminal".into())),
        }
    }
}